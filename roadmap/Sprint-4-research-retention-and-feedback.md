...
- [ ] Add a concise “Following” digest section containing material score, funding,
  hiring, or target-role changes for pinned companies.
- [ ] Add a simple manual “add company” flow: validate domain/name, detect an existing
...
- [ ] Make failure states explicit when an ATS cannot be detected; allow the company to
  remain a research note rather than pretending it has live role coverage.

### Review and archive

- [ ] Build a **Review** screen grouped by reason, with clear promote, resolve, and
  dismiss actions. Promotion requires a real domain or closed-round evidence; never
  guess a canonical domain.
- [ ] Build a **Research** archive that lists prior committed digests and renders a
...
- [ ] Retain explicit empty states for an empty review queue and a fresh installation
  with no archive.
...
- [ ] Add a short local “How this works” page/section that explains score confidence,
...
- [ ] Update `README.md` and `src/README.md` with `scan serve`, the dashboard’s local
...
- [ ] Open `http://127.0.0.1:3000` and capture initial Review and Research screenshots.
- [ ] Use `agent-browser snapshot -i` to find promotion/dismissal, archive, and add-
  company controls; exercise their primary flows with interactive refs.
- [ ] Confirm promotions, dismissals, notes, and a manually added company in
...
- [ ] Confirm the final tool remains localhost-bound and uses no new cloud services.
- [ ] Do not create AWS resources or seek cost approval because none are needed for the
